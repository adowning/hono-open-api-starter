...
export function getLatestVersion(versions: AppVersion[], type: UpdateTypeZ): AppVersion | null {
...
function generateR2Key(appId: string, platform: string, type: UpdateTypeZ, version: string): string {
  const extension = getFileExtension(type);
  return `${appId}/${platform}/${type}/${version}/bundle.${extension}`;
}

export function generateDownloadUrl(appId: string, platform: string, type: UpdateTypeZ, version: string): string {
  if (isProduction) {
...
export async function saveUpdateFile(file: File, appId: string, platform: string, type: UpdateTypeZ, version: string): Promise<{ buffer: ArrayBuffer }> {
