...
export class SessionManager {
  ...
  static async startGameSession(c: Context, gameName: string): Promise<GameSessionType> {
    ...
    console.log(chalk.bgCyan("Starting new game session for user:", user.id));

    await this.endCurrentGameSession(user.id);

    const game = await db.query.Game.findFirst({ where: eq(Game.name, gameName) });
    if (!game) {
  ...
  static async getGameSession(sessionId: string): Promise<GameSessionType | null> {
  ...
  static async handleIdleSession(c: Context): Promise<void> {
    ...
    const gameSession = await getGameSessionFromCache(user.currentGameSessionDataId);
    ...
    if (gameSession) {
      ...
      if (timeDiff > IDLE_TIMEOUT) {
        console.log(chalk.yellow(`Session ${gameSession.id} timed out due to inactivity.`));
        await this.endCurrentGameSession(user.id);
