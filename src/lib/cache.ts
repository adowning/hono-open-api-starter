...
export function getGameSessionFromCache(sessionId: string): GameSessionType | null {
...
export async function saveGameSessionToCache(session: GameSessionType, c: Context): Promise<void> {
...
export async function getSpinsFromCache(sessionId: string): Promise<Partial<GameSpinType>[]> {
...
export async function addSpinToCache(sessionId: string, spin: Partial<GameSpinType>): Promise<void> {
  const spinsKey = `${SPINS_PREFIX}${sessionId}`;
