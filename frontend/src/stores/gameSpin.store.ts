...
import { Game, getGamesAll } from '@/sdk/generated'
...
export const useGameSpinStore = defineStore('gameSpin', () => {
    ...
    //      */
    async function getTopWins(): Promise<void> {
        isLoading.value = true
        errMessage.value = ''
        try {
            const { data, error } = await getGamesAll()
            if (error || !data) throw new Error('Failed to get top wins')
            topWins.value = data
        } catch (error: any) {
            errMessage.value = handleException(error.code || 500)
            topWins.value = []
